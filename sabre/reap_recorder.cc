...
void ReapRecorder::userfaultfd_handler_record_() {
  ...
  while (do_record_) {
    ...
    if (UserfaultfdPoll(&fault_addr) == -1) {
      RLOG(LOG_ERROR) << "Userfaultfd error, exiting right now...";
      return;
    }
    ...
    if (do_write_records_) {
      RLOG(LOG_VERBAL) << "PF @" << std::hex << (void *)(fault_addr)
                       << std::dec;
...
void ReapRecorder::sock_handler_() {
  ...
  if (msgsock == -1) {
    RLOG(LOG_ERROR) << "Error accepting incoming connection.";
    return;
  }
  ...
  while (do_listen_) {
    ...
    if (read(msgsock, buff, 1024) < 0) {
      RLOG(LOG_ERROR) << "Error reading from socket.";
      return;
    }

    RLOG(LOG_INFO) << "Sock: smth received: " << std::string(buff);
    if (std::string(buff) == "RECORD") {
      RLOG(LOG_INFO) << "Starting recording.";
      recordings_.reserve(1024 * 1024);
      do_write_records_ = true;
    } else if (std::string(buff) == "STOP_RECORD") {
      RLOG(LOG_INFO) << "Stopping recording, total records: "
                     << recordings_.size();
      do_write_records_ = false;
      DumpRecordedPages();
    } else {
      RLOG(LOG_ERROR) << "Error: Unknown command received.";
      return;
    }
  }
}
...
int ReapRecorder::StartListening(uint8_t *mem, size_t size,
  ...
  if (sock_filename_ == "") {
    RLOG(LOG_ERROR) << "Failed to start listening, wrong socket filename.";
    return -1;
  }
  ...
  if (sock_ < 0) {
    RLOG(LOG_ERROR) << "Failed to create listening socket.";
    return -1;
  }
  ...
  if (bind(sock_, (struct sockaddr *)&server, sizeof(struct sockaddr_un))) {
    RLOG(LOG_ERROR) << "Failed to bind listening socket.";
    return -1;
  }
  do_listen_ = true;
  sock_listening_thread_ = std::thread(&ReapRecorder::sock_handler_, this);
  RLOG(LOG_INFO) << "Reap recorder is listening for recording commands.";
  ...
  RLOG(LOG_INFO) << "REAP is ready to record pages @" << std::hex
...
int ReapRecorder::DumpRecordedPages() {
  ...
  RLOG(LOG_INFO) << "Making Reap snapshot: initial #of partition: "
                 << recordings_.size()
                 << ", after merging: " << partitions.size();
  ...
  if (memory_restorator.Init()) {
    RLOG(LOG_ERROR) << "Failed to dump recordeed pages.";
  }
...
int ReapRecorder::Restore(uint8_t *mem, size_t size,
  ...
  acc::MemoryRestorator memory_restorator(cfg, ws_filename_, nullptr);
  if (memory_restorator.Init()) {
    RLOG(LOG_ERROR) << "Failed to install Reap pages";
    return -1;
  }
  ...
  userfaultfd_thread_ =
      std::thread(&ReapRecorder::userfaultfd_handler_record_, this);
  RLOG(LOG_INFO)
      << "REAP memory is restored, continuing execution from original "
         "spapshot.";
...
int ReapRecorder::MmapUnderlyingSnapshotFile(
  ...
  if (snapshot_fd == -1) {
    RLOG(LOG_ERROR) << "Error during snapshot file open.";
    return -1;
  }
  ...
  if (underlying_mem.get() == nullptr) {
    RLOG(LOG_ERROR) << "Error during snapshot file mmap.";
    return -1;
  }
  RLOG(LOG_INFO) << "Snapshot is mmaped, pages are going to be served from "
                 << snapshot_filename;
...
int ReapRecorder::UserfaultfdPoll(uint64_t *fault_addr) const {
  ...
  if (nread == 0) {
    RLOG(LOG_ERROR) << "EOF on userfaultfd.";
    return -1;
  }
  if (nread == -1) {
