...
void ReapRecorder::userfaultfd_handler_record_() {
  ...
  while (do_record_) {
    ...
    if (UserfaultfdPoll(&fault_addr) == -1) {
      RLOG(0) << "Userfaultfd error, exiting right now..." << std::endl;
      return;
    }
...
void ReapRecorder::sock_handler_() {
  ...
  if (msgsock == -1) {
    RLOG(0) << "Error accepting incoming connection." << std::endl;
    return;
  }
  ...
  while (do_listen_) {
    ...
    if (read(msgsock, buff, 1024) < 0) {
      RLOG(0) << "Error reading from socket." << std::endl;
      return;
    }

    RLOG(1) << "Sock: smth received: " << std::string(buff) << std::endl;
    if (std::string(buff) == "RECORD") {
      RLOG(1) << "Starting recording." << std::endl;
      ...
      RLOG(0) << "Error: Unknown command received." << std::endl;
      return;
    }
  }
}
...
int ReapRecorder::StartListening(uint8_t *mem, size_t size,
  ...
  if (sock_ < 0) {
    RLOG(0) << "Failed to create listening socket." << std::endl;
    return -1;
  }
  ...
  if (bind(sock_, (struct sockaddr *)&server, sizeof(struct sockaddr_un))) {
    RLOG(0) << "Failed to bind listening socket." << std::endl;
    return -1;
  }
  do_listen_ = true;
  sock_listening_thread_ = std::thread(&ReapRecorder::sock_handler_, this);
  RLOG(1) << "Reap recorder is listening for recording commands." << std::endl;
...
int ReapRecorder::DumpRecordedPages() {
  ...
  if (memory_restorator.Init()) {
    RLOG(0) << "Failed to dump recordeed pages." << std::endl;
  }
...
int ReapRecorder::Restore(uint8_t *mem, size_t size,
  ...
  acc::MemoryRestorator memory_restorator(cfg, ws_filename_, nullptr);
  if (memory_restorator.Init()) {
    RLOG(0) << "Failed to install Reap pages" << std::endl;
    return -1;
  }
  ...
  userfaultfd_thread_ =
      std::thread(&ReapRecorder::userfaultfd_handler_record_, this);
  RLOG(1)
      << "REAP memory is restored, continuing execution from original spapshot."
      << std::endl;
...
int ReapRecorder::MmapUnderlyingSnapshotFile(
  ...
  if (snapshot_fd == -1) {
    RLOG(0) << "Error during snapshot file open." << std::endl;
    return -1;
  }
  ...
  if (underlying_mem.get() == nullptr) {
    RLOG(0) << "Error during snapshot file mmap." << std::endl;
    return -1;
  }
